...
For a smoke run that uses only committed files, pass the toy sample explicitly:
...
The repository does not ship the full third-party or derived prompt
...
from the local `zh_lines.txt` file:
