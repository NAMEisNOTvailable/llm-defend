...
composer plus a tiny original toy sample under `source/samples/`. The public
...
in an access-controlled private export or regenerated from upstream sources
...
| Local path after preparation | Upstream source | Local use | License / access | Public repo status |
| --- | --- | --- | --- | --- |
| `source/Safety_prompt_typical_safety_scenarios.jsonl` | [thu-coai/Safety-Prompts][safety-prompts-github] / [Hugging Face][safety-prompts-hf] | Chinese safety scenario coverage for prompt composition and phrase extraction. | Apache-2.0 on the upstream GitHub and Hugging Face dataset card. | Not mirrored in public git; prepare from private/upstream export. |
| `source/Safety_prompt_instruction_attack_scenarios.jsonl` | [thu-coai/Safety-Prompts][safety-prompts-github] / [Hugging Face][safety-prompts-hf] | Instruction-attack scenario coverage for prompt composition. | Apache-2.0 on the upstream GitHub and Hugging Face dataset card. | Not mirrored in public git; prepare from private/upstream export. |
| `source/JailBench.csv`, `source/JailBench.jsonl` | [STAIR-BUPT/JailBench][jailbench-github] / [paper][jailbench-paper] | Chinese jailbreak-risk benchmark prompts used as adversarial source material. | MIT on the upstream GitHub repository. | Not mirrored in public git; use upstream or private export. |
...
| `source/load_dataset.py` | Local helper script. | Documents the MultiJail Chinese split export path. | Script is MIT with the rest of the original code; downloaded/exported data remains under upstream terms. | Script remains public; exported data remains local/private. |
